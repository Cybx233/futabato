...
### `background.js`
Service worker，职责很轻：
1. 监听 `chrome.action.onClicked`，工具栏图标点击时发送 `{ action: 'toggle' }` 消息。
2. 监听 `chrome.commands.onCommand`，响应用户快捷键（`Cmd+Shift+Y` / `Ctrl+Shift+Y`）切换阅读模式。
...
- **方向感知**：RTL / LTR 切换会同时影响翻页方向、点击区映射、进度条填充方向，以及合集前后篇按钮的左右位置。
- **跨页偏移**：`toggleOffset` 在首页插入空白占位，用于重新对齐左右页；激活时按钮显示绿色线框高亮。
- **合集检测**：`_detectCollection` 直接定位微信 `.album_read_nav_prev` / `.album_read_nav_next` 的 `<span role="button">` 元素，点击时模拟原生点击实现跨文章导航。
- **滚轮翻页**：`_handleWheel` 采用即时响应 + 动画锁（200ms）机制，每次滚轮事件立即翻页，翻页动画期间忽略后续事件。
